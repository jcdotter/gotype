...
var (
	...
	struct_struct = struct_type{"0", "1"}
	struct_blank  = struct_type_{}
	struct_cmp    = struct_complex{bool_bool, float_float, time_time, uuid_uuid, string_string, array_array, map_map, struct_struct, struct_blank, json_array}
...
func TestValidations(t *testing.T) {
	var validations = []validation_test{
		...
		{"Conversion: Struct to String", ValueOf(struct_struct).STRUCT().JsonByTag("json").String(), string(json_json)},
		...
		{"Conversion: Struct to Slice", ValueOf(struct_struct).SLICE().Interface(), array_array},
		{"Conversion: Struct to Strings Slice", ValueOf(struct_struct).STRUCT().Strings(), []string{"0", "1"}},
		...
		{"Conversion: Struct to Map", ValueOf(struct_struct).STRUCT().MapByTag("json"), map_map},
		{"Conversion: Json to Map", ValueOf(json_json).MAP().Index("1").String(), map_map["1"].(string)},

		{"Conversion: Slice to Struct", ValueOf(array_array).SLICE().StructScan(StructOf(struct_type{})).Interface(), struct_struct},
		{"Conversion: Map to Struct", ValueOf(map_map).MAP().StructScan(StructOf(struct_type{}), "json", "").Interface(), struct_struct},
		{"Conversion: Struct to Struct", ValueOf(struct_struct).STRUCT().Interface(), struct_struct},
		{"Conversion: Json to Struct", ValueOf(json_json).JSON().StructScan(StructOf(struct_type{}), "json", "").Interface(), struct_struct},
		{"Conversion: Empty Struct to Struct", ValueOf(&struct_blank).Elem().Set(struct_struct).STRUCT().MapByTag("json"), map_map},
		{"Conversion: Struct to Json", ValueOf(struct_struct).STRUCT().SerializeByTag("json"), string_json},
		...
		{"Conversion: Struct to Json", ValueOf(struct_struct).STRUCT().JsonByTag("json"), json_json},
	...
	validations = []validation_test{
		...
		{"Set Value: Set Struct Pointer", StructOf(&struct_struct).Index(0).Set(2).Interface(), "2"},
